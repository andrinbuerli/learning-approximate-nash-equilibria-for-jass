...
class BufferingNetwork(AbstractNetwork):
    def __init__(self, network: AbstractNetwork, buffer_size: int):
        super().__init__()
    ...
    def _check_initial_queue(self):
        ...
        while self._running:
            observation, con = self._initial_queue.get()
            observations.append(observation)
            conns.append(con)

            if len(observations) >= self.buffer_size:
    ...
    def _check_recurrent_queue(self):
        ...
        while self._running:
            (o, a), con = self._recurrent_queue.get()
            encoded_states.append(o)
            actions.append(a)
            conns.append(con)

            if len(encoded_states) >= self.buffer_size:
    ...
    def initial_inference(self, observation, return_connection=False):
        assert len(observation.shape) == 4, "observation must be batched"
    ...
    def recurrent_inference(self, encoded_state, action, return_connection=False):
        assert len(encoded_state.shape) == 4, "encoded_state must be batched"
